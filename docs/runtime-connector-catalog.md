...
| `cursor_cli_external_loop` | Cursor CLI external tick driver | Local scheduler loop (`loopx agent-start --agent-type cursor-cli`) | `loopx agent-start`; loop re-invokes tick worker at `LOOPX_CURSOR_TICK_INTERVAL`; scheduler_hint backoff via `quota should-run` | `todo` lifecycle, `refresh-state`, then `quota spend-slot` after `cursor-agent` validated work | Loop script stdout and compact quota status; `LOOPX_CURSOR_REQUIRE_WRITEBACK` spent_slots increase | Apply `scheduler_hint`; exit code 2 stops on config error; quota pause (exit 75) retries with `LOOPX_CURSOR_PAUSE_INTERVAL` backoff; `LOOPX_CURSOR_MAX_TICKS` limits total ticks | Quota slot after verified writeback (spent_slots increase); no spend for quota-paused or config-error ticks | User sees `cursor-agent` output and loop status in terminal | Managed-marker required before exec; shared registry (`LOOPX_GLOBAL_REGISTRY`) for `should-run`/`spend`; no local paths baked into scripts; `cursor-agent` invoked without unsandboxed flags | Behavioral smoke covers should-run gate, config-failure stop (not pause-loop), writeback verify, managed-marker check, and shared registry selection. |
