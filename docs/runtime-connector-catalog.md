...
| `cursor_cli_external_loop` | Cursor CLI external tick driver | Local scheduler loop (`loopx agent-start --agent-type cursor-cli`) | `loopx agent-start`; loop re-invokes tick worker at `LOOPX_CURSOR_TICK_INTERVAL`; scheduler_hint backoff via `quota should-run` | Cursor rule prompts `cursor-agent` to run `refresh-state`, `todo complete`, then `quota spend-slot`; tick worker verifies quota accounting (`spent_slots` increase) as a proxy signal — todo/evidence/state writeback relies on prompt compliance | Loop script stdout and compact quota status; `LOOPX_CURSOR_REQUIRE_WRITEBACK` verifies `spent_slots` increase as a best-effort proxy (not full state writeback) | Apply `scheduler_hint`; exit code 2 stops on config error; quota pause (exit 75) retries with `LOOPX_CURSOR_PAUSE_INTERVAL` backoff; `LOOPX_CURSOR_MAX_TICKS` limits total ticks | Quota slot after `spent_slots` increase verified; no spend for quota-paused or config-error ticks | User sees `cursor-agent` output and loop status in terminal | Managed-marker required on both loop and tick worker scripts before exec; shared registry (`LOOPX_GLOBAL_REGISTRY`) for `should-run`/`heartbeat-prompt`/`spend`; no local paths baked into scripts; `cursor-agent` invoked without unsandboxed flags | Behavioral smoke covers should-run gate, config-failure stop (not pause-loop), writeback proxy check, managed-marker check on loop and tick worker, and shared registry selection. |
